...
def test_windows_powershell_operator_scripts_are_wired():
    ...
    assert '$ErrorActionPreference = "Stop"' in scripts["ZtCommon.ps1"]
    ...
    assert "ResourceNotFoundException" in scripts["Test-ZtPreflight.ps1"]
    assert "ResourceNotFoundException" in scripts["New-ZtTailscaleSecret.ps1"]
    assert '$ErrorActionPreference = "Continue"' in scripts["Test-ZtPreflight.ps1"]
    assert '$ErrorActionPreference = "Continue"' in scripts["New-ZtTailscaleSecret.ps1"]
    assert "$errorString" in scripts["Test-ZtPreflight.ps1"]
    assert "$errorString" in scripts["New-ZtTailscaleSecret.ps1"]
    ...
    assert "describe-instance-information failed with exit code" in scripts["Get-ZtLogs.ps1"]
    assert '$ErrorActionPreference = "Continue"' in scripts["Get-ZtLogs.ps1"]
    assert 'Where-Object { $_ -is [string] }' in scripts["Get-ZtLogs.ps1"]
    assert "$parametersUri" in scripts["Get-ZtLogs.ps1"]
    assert ".Replace('\\', '/')" in scripts["Get-ZtLogs.ps1"]
